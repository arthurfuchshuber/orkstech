...
import { commitImportTargeted, type ImportTarget, TARGET_LABELS } from "@/lib/import-targets";
import { useAuth } from "@/hooks/useAuth";
import { useEmpresa } from "@/hooks/useEmpresa";
import { cn } from "@/lib/utils";
import { MappingConfirmDialog } from "../fluxo/MappingConfirmDialog";
...
interface Props {
  target: ImportTarget;
  /** For bank_statement: optional default bank account to associate imported rows */
  bankAccountId?: string;
  onImported?: () => void;
}

export function GenericImporter({ target, bankAccountId, onImported }: Props) {
  const { user } = useAuth();
  ...
  const handleCommit = async (includeDuplicates: boolean) => {
    ...
    setLoading(true);
    try {
      const res = await commitImportTargeted({
        userId: user.id,
  ...
  // Direction-based filtering: hide irrelevant counts depending on target
  const relevantValid = preview
    ? target === "payable"
      ? preview.valid.filter((r) => r.direction === "outflow")
      : target === "receivable"
        ? preview.valid.filter((r) => r.direction === "inflow")
        : preview.valid
    : [];

  const targetHint =
    target === "payable"
      ? "Apenas linhas com valor de saída (despesas) serão importadas como contas a pagar."
      : target === "receivable"
        ? "Apenas linhas com valor de entrada (receitas) serão importadas como contas a receber."
        : target === "bank_statement"
          ? "Lançamentos serão registrados como movimentações reais no extrato bancário."
          : "Lançamentos serão registrados como previsões no fluxo de caixa.";

  return (
    <Card className="border-border/50">
      <CardHeader>
        <CardTitle className="text-base font-semibold">
          Importar — {TARGET_LABELS[target]}
        </CardTitle>
        <p className="text-xs text-muted-foreground">
          Aceita CSV, Excel (.xlsx) ou link público do Google Sheets. Colunas reconhecidas:{" "}
          <strong>data, valor, descrição, documento, categoria, tipo, observações</strong>.
        </p>
        <p className="text-xs text-primary/80 mt-1">{targetHint}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {!preview && (
          <Tabs value={tab} onValueChange={(v) => setTab(v as any)}>
            ...
            <TabsContent value="sheets" className="mt-4 space-y-3">
              <Label htmlFor="sheetsUrl" className="text-sm">
        ...
        {preview && (
          <div className="space-y-4">
            {(preview.detectedColumns?.dateKey ||
              preview.detectedColumns?.amountKey ||
              preview.detectedColumns?.descKey) && (
              <div className="p-3 rounded-lg bg-primary/5 border border-primary/20">
                ...
                <div className="flex flex-wrap gap-2 text-[11px]">
                  {preview.detectedColumns.dateKey && (
                    <Badge variant="outline">Data: {preview.detectedColumns.dateKey}</Badge>
                  )}
                  {preview.detectedColumns.amountKey && (
                    <Badge variant="outline">Valor: {preview.detectedColumns.amountKey}</Badge>
                  )}
                  {preview.detectedColumns.descKey && (
                    <Badge variant="outline">Descrição: {preview.detectedColumns.descKey}</Badge>
                  )}
                </div>
              </div>
            )}
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/20">
                ...
                <p className="text-2xl font-bold text-foreground">{relevantValid.length}</p>
                <p className="text-[11px] text-muted-foreground">
                  {fmt(relevantValid.reduce((s, r) => s + r.amount, 0))}
                </p>
              </div>
              ...
              <div className="border border-amber-500/20 rounded-lg p-3 bg-amber-500/5 max-h-64 overflow-auto">
                ...
                <div className="space-y-1">
                  {preview.duplicates.slice(0, 20).map((d) => (
                    <div
                      key={d.rowIndex}
                      className="text-xs flex items-center justify-between gap-2 py-1 border-b border-amber-500/10 last:border-0"
                    >
                      <span className="text-foreground truncate">
                        Linha {d.rowIndex}: {d.description}
                      </span>
                      <Badge variant="outline" className="text-[10px] flex-shrink-0">
                        já em{" "}
                        {d.duplicateOf.table.replace("accounts_", "").replace("cashflow_forecasts", "previsão")}
                      </Badge>
                    </div>
                  ))}
                  {preview.duplicates.length > 20 && (
                    <p className="text-[11px] text-muted-foreground pt-1">
                      + {preview.duplicates.length - 20} outras
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="border border-rose-500/20 rounded-lg p-3 bg-rose-500/5 max-h-48 overflow-auto">
                ...
                <div className="space-y-1">
                  {preview.invalid.slice(0, 10).map((d) => (
                    <p key={d.rowIndex} className="text-xs text-foreground">
                      Linha {d.rowIndex}: {d.errors?.join(", ")}
                    </p>
                  ))}
                </div>
              </div>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button
                ...
                disabled={loading || relevantValid.length === 0}
              >
                {loading ? (
                  <Loader2 className="w-4 h-4 animate-spin mr-2" />
                ) : (
                ...
                Importar {relevantValid.length} válidas
              </Button>
            </div>
          </div>
        )}
