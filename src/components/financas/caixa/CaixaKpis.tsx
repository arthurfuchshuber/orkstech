  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {cards.map((c) => {
        const Icon = c.icon;
        return (
          <Card key={c.label} className="border-border/50 hover:border-border transition-colors relative">
            {c.flag && (
              <span className={cn(
                "absolute top-2 right-2 text-[9px] uppercase tracking-wider font-medium px-1.5 py-0.5 rounded border",
                flagStyles[c.tone]
              )}>
                {c.flag}
              </span>
