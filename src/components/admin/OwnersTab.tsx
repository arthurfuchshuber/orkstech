...
export function OwnersTab({ users, niveis, isLoading }: Props) {
  const { user } = useAuth();
  ...
  const updateUserMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Usuário atualizado"); setEditingUser(null); qc.invalidateQueries({ queryKey: ["admin-all-users"] }); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const updateCompanyMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Empresa atualizada"); setEditingCompany(null); qc.invalidateQueries({ queryKey: ["admin-all-users"] }); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const deleteCompanyMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Empresa excluída"); setEditingCompany(null); qc.invalidateQueries({ queryKey: ["admin-all-users"] }); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-4">
      <div className="relative max-w-sm">
        ...
        <Input placeholder="Buscar por nome, e-mail, empresa..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 h-9 text-sm" />
      </div>
          ...
          <TableBody>
            {isLoading ? (
              ...
              ownerUsers.map((u) => {
                ...
                return (
                  <>{/* User row */}
                    <TableRow key={u.id} className={cn("cursor-pointer hover:bg-muted/50 transition-colors border-border/20", !u.ativo && "opacity-50")} onClick={() => empresaCount > 0 && toggleExpanded(u.id)}>
                      <TableCell className="pr-0">
                        {empresaCount > 0 && <ChevronRight className={cn("w-4 h-4 text-muted-foreground transition-transform duration-200", isExpanded && "rotate-90")} />}
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center gap-0.5">
                          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openEditUser(u)}><Pencil className="w-3.5 h-3.5" /></Button>
                          {!isSelf && (
                            <AlertDialog>
                    ...
                    {isExpanded && u.empresas.map((emp) => (
                      <TableRow key={emp.id} className="bg-muted/20 border-border/10 hover:bg-muted/30">
                        ...
                        <TableCell className="text-xs text-muted-foreground font-mono">{emp.cnpj}</TableCell>
                        <TableCell className="text-xs text-muted-foreground whitespace-nowrap">{format(new Date(emp.created_at), "dd/MM/yyyy")}</TableCell>
                        <TableCell></TableCell>
                        <TableCell>
                          <div className="flex items-center gap-0.5">
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openEditCompany(emp)}><Pencil className="w-3.5 h-3.5" /></Button>
                            <AlertDialog>
                              <AlertDialogTrigger asChild><Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive"><Trash2 className="w-3.5 h-3.5" /></Button></AlertDialogTrigger>
                              <AlertDialogContent>
                                <AlertDialogHeader><AlertDialogTitle>Excluir empresa</AlertDialogTitle><AlertDialogDescription>Tem certeza que deseja excluir <strong>{emp.nome_fantasia || emp.razao_social}</strong>?</AlertDialogDescription></AlertDialogHeader>
                                <AlertDialogFooter><AlertDialogCancel>Cancelar</AlertDialogCancel><AlertDialogAction onClick={() => deleteCompanyMutation.mutate(emp.id)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Excluir</AlertDialogAction></AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          </div>
                        </TableCell>
      ...
      <Dialog open={!!editingUser} onOpenChange={(open) => !open && setEditingUser(null)}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => updateUserMutation.mutate()} disabled={updateUserMutation.isPending}>{updateUserMutation.isPending ? "Salvando..." : "Salvar"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
