...
**IA:** edge function `classify-tipos-gasto` recebe `{empresa_id, only_uncategorized}`, varre saídas sem tipo_gasto_id em accounts_payable + pluggy_transactions + manual_bank_transactions + cash_transactions, chama Lovable AI (gemini-3-flash-preview) com tool calling, aplica retroativamente. Botão "Classificar via IA" no header de Extrato Bancário e Contas a Pagar.
