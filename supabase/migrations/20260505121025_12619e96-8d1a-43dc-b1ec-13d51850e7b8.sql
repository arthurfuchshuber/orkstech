...
BEGIN
  ...
  INSERT INTO public.menus (user_id, name, slug, icon, route, parent_id, order_index, module) VALUES
    ...
    (p_user_id, 'Recursos Humanos', 'rh-cadastros', 'Settings2', '/app/rh/cadastros', v_cadastros, 3, 'cadastros');
...
BEGIN
  ...
  -- Cadastros RH agora vai dentro do grupo "Cadastros"
  SELECT id INTO v_cadastros FROM public.menus
  WHERE user_id = p_user_id AND slug = 'cadastros' AND parent_id IS NULL
  LIMIT 1;

  IF v_cadastros IS NOT NULL THEN
    INSERT INTO public.menus (user_id, name, slug, icon, route, parent_id, order_index, module) VALUES
      (p_user_id, 'Recursos Humanos', 'rh-cadastros', 'Settings2', '/app/rh/cadastros', v_cadastros,
       COALESCE((SELECT MAX(order_index)+1 FROM public.menus WHERE parent_id = v_cadastros), 3), 'cadastros');
  END IF;

  INSERT INTO public.menu_permissions (menu_id, role, can_view)
  SELECT id, 'admin', true FROM public.menus WHERE user_id = p_user_id AND module IN ('rh','cadastros') AND slug LIKE 'rh-%'
  ON CONFLICT DO NOTHING;
  INSERT INTO public.menu_permissions (menu_id, role, can_view)
  SELECT id, 'user', true FROM public.menus WHERE user_id = p_user_id AND module IN ('rh','cadastros') AND slug LIKE 'rh-%'
  ON CONFLICT DO NOTHING;
END;
$function$;