...
BEGIN
  ...
  FOR v_cond IN SELECT * FROM jsonb_array_elements(p_condicoes) LOOP
    ...
    ELSE
      CASE v_op
        WHEN 'contains' THEN
          -- Matching tolerante: normaliza ambos os lados (sem acento/pontuação)
          ...
          ELSIF position(v_val_norm in v_field_value_norm) > 0 THEN
            -- Match direto da frase normalizada inteira
            v_match := true;
          ELSE
            -- Fallback: pelo menos 60% dos tokens significativos (3+ chars) precisam aparecer
            v_total_tokens := 0;
            v_matched_tokens := 0;
            FOR v_token IN SELECT unnest(string_to_array(v_val_norm, ' ')) LOOP
              IF length(v_token) >= 3 THEN
                ...
                IF position(v_token in v_field_value_norm) > 0 THEN
                  v_matched_tokens := v_matched_tokens + 1;
                END IF;
              END IF;
            END LOOP;
            IF v_total_tokens = 0 THEN
              v_match := false;
            ELSE
              v_match := (v_matched_tokens::numeric / v_total_tokens::numeric) >= 0.6;
            END IF;
          END IF;
        WHEN 'equals' THEN
