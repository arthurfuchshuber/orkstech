-- Inserir item "Sincronização" no grupo Financeiro de todos os usuários que ainda não possuem
INSERT INTO public.menus (user_id, name, slug, icon, route, parent_id, order_index, module)
SELECT m.user_id, 'Sincronização', 'sincronizacao', 'RefreshCw', '/app/financas/sincronizacao', m.id, 6, 'financeiro'
FROM public.menus m
WHERE m.slug = 'financeiro' AND m.parent_id IS NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.menus c
    WHERE c.user_id = m.user_id AND c.slug = 'sincronizacao'
  );

-- Permissões padrão
INSERT INTO public.menu_permissions (menu_id, role, can_view)
SELECT id, 'admin', true FROM public.menus WHERE slug = 'sincronizacao'
ON CONFLICT DO NOTHING;
INSERT INTO public.menu_permissions (menu_id, role, can_view)
SELECT id, 'user', true FROM public.menus WHERE slug = 'sincronizacao'
ON CONFLICT DO NOTHING;

-- Atualiza o seed para novos usuários
...
BEGIN
  ...
  INSERT INTO public.menus (user_id, name, slug, icon, route, parent_id, order_index, module) VALUES
    ...
    (p_user_id, 'Sincronização', 'sincronizacao', 'RefreshCw', '/app/financas/sincronizacao', v_financeiro, 6, 'financeiro');
